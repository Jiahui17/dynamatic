...
# NOTE: Here is a brief summary of what each llvm pass does:
...
# - sccp: prove and replace SSA values by constants
# - sink: move instructions into successor blocks
# - consthoist: moving constants around
# - dce: dead code elimination
# - loop-simplify: transform natual loops into simpler forms
# - simplifycfg: merge BBs
# 
# NOTE: It is unclear how to order the passes.
# 
...
$LLVM_BINS/opt -S \
  -passes="mem2reg,instcombine,loop-rotate,sccp,sink,consthoist,dce,loop-simplify,simplifycfg" \
  -strip-debug \
  $OUT/clang.ll \
